...
最终运行 `v2auth20260828f` 已在代码 `d5707c22c645`、PIT 数据指纹
`faf588403e3db351`、组合配置 `c2770dc24fd37c4b` 上完成。最终结果为
`FAIL / NO_CANDIDATE`：

- OOS：172 笔完整退出，组合净收益 `8.493586%`、PF `1.22260136`、最大回撤
  `9.182442%`，组合状态 PASS。
- 主基线：MA20/60 为 `-14.566542%`；固定种子随机为 `-16.923076%`。
- 2×成本：候选 `+4.843727%`、MA20/60 `-19.064983%`，双方均完整结算。
- 参数邻域：计划 7 组、评估 7 组、覆盖率 100%，7/7 均优于预登记主基线。
- 正式阻断：PBO `0.3125 > 0.20`，DSR `0.463596 < 0.95`，MinTRL 覆盖
  `0.188 < 1`，嵌套参数测试正收益窗 `1/5 < 60%`。
- 数据库与产物一致：`status=done`、`verdict=FAIL`、`candidate_eligible=0`、
  `can_claim_edge=0`；Markdown SHA-256 为
  `b2500df652b5dfe4916aba62e03440493dae0fd2a99f3d7177c07491fd920e81`，未生成研究候选。

该运行现作为当前权威失败事实；旧运行保留且不覆盖。若继续提高策略，只能提出新经济假设并
重新预登记，不能基于本次 OOS 结果修改参数网格或门槛。
