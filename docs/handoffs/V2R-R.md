...
> **管理者复验更正（2026-08-24，具有最高优先级）**
>
> 初版任务 `1699927499ff` 及本文下方对应数字已经作废，仅保留为审计历史。独立复验发现旧优化器会先利用突破日后的形态确认数据，再把信号与入场回填到突破日/次日，构成未来函数。修复提交为 `eb08e9986bac6f70684b3e12e19eeed0b1f7c77a`，新增因果窗口重放与 `(code, breakout_date)` 去重测试。
>
> 唯一有效的冻结研究任务为 `0746a4108e15`：600 股、step=5、979 个交易日，`verdict=FAIL`、`candidate_eligible=false`。OOS 共 422 笔净成交，净 PF `1.112`、胜率 `41.0%`、最大回撤 `88.03%`；WF 净 PF 为 `0.613 / 0.730 / 1.240`，仅 1/3 盈利；2 倍成本 PF `0.986`，3 倍成本 PF `0.875`；PBO `0.3815`（FAIL），DSR `0`，MinTRL `1762.49`。因果回放中“入场不晚于信号”“信号/突破日错位”“同标的同突破日重复”计数均为 `0`。
>
> 权威证据目录：`runtime/v2/research_v2r_r/evidence/`。`sha256_manifest.json` 对四个证据文件复算完全匹配；`verify_metrics.json` 的数据集指纹、成本版本、回放指标、门禁结果与候选资格均与冻结结果一致。未登记候选、未进入 A 池、未创建订单。
>
> **本文以下原 Agent 自报内容是被更正前的历史记录，不得再作为验收或策略结论引用。**
