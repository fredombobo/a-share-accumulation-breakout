...
def eval_combo(combo: dict, start: str, end: str, step: int = 5,
               max_codes: int | None = None, progress_cb=None, cancel_check=None,
               signal_kwargs: dict | None = None, costs: dict | None = None) -> dict:
    ...
    df = run_grid(start=start, end=end, strategy=combo["strategy"], step=step,
                  ...
                  cancel_check=cancel_check, signal_kwargs=signal_kwargs, costs=costs)
    if df.empty:
        return {"n_trades": 0}
...
def run_is_oos(strategy: str, step: int = 5, max_codes: int | None = None,
    ...
    if single:
        ...
        oos_df = pd.DataFrame([{
            ...
            **{f"is_{k}": is_row.get(k) for k in (
                "n_trades", "win_rate", "profit_factor", "max_drawdown",
                "net_n_trades", "net_win_rate", "net_profit_factor", "net_max_drawdown",
            )},
        }])
        if progress_cb:
    ...
    is_df = run_grid(
        start=is_start, end=is_end, strategy=strategy, step=step,
        max_codes=max_codes, grid=grid,
        progress_cb=_phase_progress(progress_cb, "IS", 0, 50),
        cancel_check=cancel_check,
...
def wf_recheck(combos: list[dict], step: int = 5, max_codes: int | None = None,
               progress_cb=None, windows: list[tuple] | None = None,
               cancel_check=None, signal_kwargs: dict | None = None,
    ...
    for combo in combos:
        ...
        for wname, (ts, te, vs, ve) in zip(("WF1", "WF2", "WF3"), wf_windows):
            ...
            train = eval_combo(
                combo, ts, te, step=step, max_codes=max_codes,
                cancel_check=cancel_check, signal_kwargs=signal_kwargs, costs=costs,
            )
            ...
            test = eval_combo(
                combo, vs, ve, step=step, max_codes=max_codes,
                cancel_check=cancel_check, signal_kwargs=signal_kwargs, costs=costs,
            )
            completed_evaluations += 1
            if progress_cb:
                progress_cb(f"{wname} 完成", int(100 * completed_evaluations / total_evaluations))
            tests.append({"window": wname, "train_pf": train.get("net_profit_factor"),
        ...
        evidence_complete = len(tests) == 3 and all(
            ...
            for t in tests
            for key in ("train_pf", "test_pf", "test_dd", "test_n")
        )
        ...
        ratio_ok = bool(
            evidence_complete and train_mean is not None and train_mean > 0
