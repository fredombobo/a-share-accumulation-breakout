...
def test_markdown_report_contains_reproducibility_and_all_gate_sections() -> None:
    ...
    for heading in ("结论", "样本与版本", "成本口径", "IS / OOS", "Walk-forward", "基线对照", "反过拟合", "门禁检查", "敏感性"):
...
def test_orchestrator_runs_all_stages_and_builds_pass_report(monkeypatch, tmp_path) -> None:
    ...
    combo = {
        ...
        "net_n_trades": 40, "net_profit_factor": 1.4,
        "net_win_rate": 0.4, "net_max_drawdown": 0.2,
    }
    oos = {
        "param_id": "p1",
        **{key: combo[key] for key in ("strategy", "vol_ratio_min", "strong_reset", "exit_window", "stop_pct")},
        "oos_net_n_trades": 40, "oos_net_profit_factor": 1.2,
        "oos_net_win_rate": 0.4, "oos_net_max_drawdown": 0.2,
    ...
    monkeypatch.setattr(
        ...
        lambda **_kwargs: {
            ...
            "oos": pd.DataFrame([
                oos,
                {**oos, "param_id": "p2", "oos_net_profit_factor": 1.1,
                 "oos_net_avg_return": 0.009},
                {**oos, "param_id": "p3", "oos_net_profit_factor": 1.05,
                 "oos_net_avg_return": 0.006},
            ]),
            "msg": None,
        },
    )
    wf_detail = [
        {"window": f"WF{i}", "train_pf": 1.2, "test_pf": 1.05, "test_dd": 0.2, "test_n": 35}
        for i in range(1, 4)
    ]
    monkeypatch.setattr(
    ...
    baseline = {"n_trades": 40, "net_avg_return": 0.001, "net_profit_factor": 1.0, "net_max_drawdown": 0.1}
    ...
    phases: list[str] = []
    windows = {
        "is_start": "20230801", "is_end": "20240731",
        "oos_start": "20240801", "oos_end": "20260731",
        "mode": "full", "automatic_window": True,
        "wf_windows": [
            {"train_start": "1", "train_end": "2", "test_start": "3", "test_end": "4"}
        ] * 3,
    }

    result = execute_trusted_research(
        research_run_id="flow", request={"strategy": "A", "mode": "grid", "max_codes": 200},
        windows=windows, db_path=db, code_version="c", dataset_version="d",
