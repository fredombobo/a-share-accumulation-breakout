...
def _wf() -> list[dict]:
    return [
        {"window": f"WF{i}", "train_pf": 1.20, "test_pf": 1.05,
         "test_dd": 0.20, "test_n": 35}
        for i in range(1, 4)
    ]
...
def _anti_overfit(**overrides) -> dict:
    result = {
        "verdict": "PASS",
        "checks": [
            {"id": "anti_trials", "label": "参数试验数量", "passed": True,
             "actual": 54, "threshold": ">= 30"},
        ],
        "block_reasons": [],
...
def test_manual_or_single_runs_are_insufficient_evidence() -> None:
    manual = evaluate_trusted_gate(
        research_mode="manual", automatic_window=False, run_mode="grid",
        oos=_oos(), wf_windows=_wf(), baselines=_baselines(), anti_overfit=_anti_overfit(),
    )
    single = evaluate_trusted_gate(
        research_mode="full", automatic_window=True, run_mode="single",
        oos=_oos(), wf_windows=_wf(), baselines=_baselines(), anti_overfit=_anti_overfit(),
    )
...
def test_missing_wf_value_never_defaults_to_zero_or_passes() -> None:
    ...
    result = evaluate_trusted_gate(
        research_mode="full", automatic_window=True, run_mode="grid",
        oos=_oos(), wf_windows=windows, baselines=_baselines(), anti_overfit=_anti_overfit(),
    )
...
def test_candidate_must_beat_random_and_ma_baselines() -> None:
    ...
    result = evaluate_trusted_gate(
        research_mode="full", automatic_window=True, run_mode="grid",
        oos=_oos(), wf_windows=_wf(), baselines=baselines, anti_overfit=_anti_overfit(),
    )
...
def test_oos_trade_and_drawdown_boundaries_are_enforced() -> None:
    result = evaluate_trusted_gate(
        ...
        wf_windows=_wf(), baselines=_baselines(), anti_overfit=_anti_overfit(),
    )
...
def test_missing_anti_overfit_evidence_is_insufficient() -> None:
    result = evaluate_trusted_gate(
        research_mode="full", automatic_window=True, run_mode="grid",
        oos=_oos(), wf_windows=_wf(), baselines=_baselines(), anti_overfit=None,
    )
...
def test_failed_anti_overfit_gate_keeps_candidate_fail_closed() -> None:
    result = evaluate_trusted_gate(
        research_mode="full", automatic_window=True, run_mode="grid",
        oos=_oos(), wf_windows=_wf(), baselines=_baselines(),
        anti_overfit=_anti_overfit(
            verdict="FAIL",
            checks=[{"id": "anti_retention", "label": "OOS PF保持率", "passed": False,
                     "actual": 0.4, "threshold": ">= 0.75"}],
...
def test_personal_anti_overfit_passes_only_with_stable_frozen_winner() -> None:
    is_rows = [
        {"param_id": f"p{i}", "net_profit_factor": 1.4 - i * 0.002}
        for i in range(54)
    ]
    oos_rows = [
    ...
    result = evaluate_personal_anti_overfit(
        is_candidates=is_rows, oos_candidates=oos_rows, wf_windows=_wf(),
    )
...
def test_personal_anti_overfit_missing_trials_is_insufficient() -> None:
    result = evaluate_personal_anti_overfit(
        is_candidates=[], oos_candidates=[], wf_windows=[],
    )
