    ...
    *,
    ...
    dedupe = hashlib.sha256(
        json.dumps({"type": alert_type, "source": source, "date": trade_date,
                    "payload": payload}, sort_keys=True, ensure_ascii=False).encode("utf-8")
    ).hexdigest()[:16]
    ...
    conn.execute(
        ...
        (alert_id, alert_type, source, trade_date, severity,
         json.dumps(payload, ensure_ascii=False, sort_keys=True, default=str), dedupe, _now()),
    )
    conn.commit()
    return alert_id
