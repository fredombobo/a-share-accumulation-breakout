...
3. 对每个组合：只冻结 train 期 Sharpe 第一名；计算其
   logit = ln(rank_test / (N - rank_test))，rank_test 为该列在 test 期的性能排名。
...
def cscv_pbo(
    ...
    if n < 4:
        raise PboError(f"参数组合数 {n} < 4（CSCV 至少需要 4 个可比较组合）")
    ...
    logits: list[float] = []
    combo_count = 0
    # 对称组合采用确定性等距抽取，限定组合数上限防止高分块数爆炸。
    max_combos = 2000
    all_combos = list(itertools.combinations(range(n_splits), half))
    if len(all_combos) > max_combos:
        indexes = np.linspace(0, len(all_combos) - 1, num=max_combos, dtype=int)
        all_combos = [all_combos[int(index)] for index in indexes]
    ...
    return {
        ...
        "selection_rule": "train_sharpe_rank_1",
        "combination_sampling": "all_or_deterministic_even_2000",
    }
