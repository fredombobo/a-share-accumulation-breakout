...
def run_single_backtest(
    *,
    ...
    if len(codes) < 100 or nw <= 1:
        ...
        results = _worker_chunk((codes, big, sample_days, cal, 160, strategy,
                                 vr_levels, combos, signal_kwargs, costs))
        if progress_cb:
    ...
    net = summarize_costed_trades(trades_raw)

    # 权益曲线（按信号日排序的保守复利，与可信管线同口径）
    equity: list[dict[str, Any]] = []
    filled = [t for t in trades_raw if t.get("cost", {}).get("filled")]
    ordered = sorted(filled, key=lambda r: (str(r.get("date") or ""), str(r.get("ts_code") or "")))
    eq = 1.0
    peak = 1.0
    for r in ordered:
        eq *= 1.0 + float(r["cost"]["net_return"])
        peak = max(peak, eq)
        equity.append({
            "date": r.get("date") or "",
