...
当前发布：[2026-09-19 个人研究开发快照与证据边界](docs/GITHUB-SNAPSHOT-2026-09-19.md)。
历史验收：[个人研究收束记录](docs/PERSONAL-RESEARCH-CLOSEOUT-2026-09-05.md)。
...
所有启动入口统一使用项目 `.venv312`（Python 3.12），不会因系统 PATH 指向 3.14 而切换研究运行时。首次从已安装的 3.12 创建环境；已有依赖通过本地检查则跳过联网安装。`mplfinance` 明确固定为已验收的 `0.12.10b0`。启动失败时保留具体错误，不需要删除数据库或重新填写已有 Token。
